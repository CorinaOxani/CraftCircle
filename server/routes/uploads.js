...
router.post("/upload-profile", upload.single("file"), async (req, res) => {
  try {
    ...
    // Update poza de profil în baza de date
...
// 🔹 Upload pentru postări (cu fișiere multiple)
router.post("/upload-post", upload.array("files", 10), async (req, res) => {
    try {
        ...
        // 🔹 2. Inserează fișierele în `post_media`
...
// 🔹 Obține toate postările și fișierele asociate
// 🔹 Obține toate postările unui utilizator și fișierele asociate
